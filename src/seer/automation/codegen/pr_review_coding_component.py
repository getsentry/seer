...
class PrReviewCodingComponent(BaseComponent[CodePrReviewRequest, CodePrReviewOutput]):
    ...
    def invoke(
        ...
        with BaseTools(self.context, repo_client_type=RepoClientType.CODECOV_PR_REVIEW) as tools:
            ...
            return format_output(final_response)

def format_output(llm_output: str) -> CodePrReviewOutput | None:
    ...
    for comment_data in comments_data:
        if not all(key in comment_data for key in ["path", "line", "body", "start_line"]):
            raise ValueError(f"Invalid comment format: {comment_data}")
    ...
    return CodePrReviewOutput(diffs=comments)
