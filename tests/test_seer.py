...
from seer.seer import aggregate_anomalies, app, predict, process_output


class TestSeer(unittest.TestCase):
    def test_severity_score(self):
        with mock.patch("seer.seer.embeddings_model") as mock_embeddings_model:
            mock_embeddings_model.severity_score.return_value = 0.5

            response = app.test_client().post(
                "/issues/severity-score",
                data=json.dumps({"message": "FATAL: App has crashed"}),
                content_type="application/json",
            )

            data = json.loads(response.data)
            self.assertIn("severity", data)
            self.assertEqual(data["severity"], "0.5")
