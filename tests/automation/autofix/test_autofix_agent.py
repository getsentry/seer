...
@patch("seer.automation.autofix.autofix_agent.AutofixAgent._get_completion")
def test_get_completion_retries_for_retryable_errors_fallback_config(
    mock_get_completion, autofix_agent, fallback_run_config
):
    """Test that get_completion retries with fallback models configuration."""
    from anthropic import AnthropicError
    ...
    # Call the get_completion method
    result = autofix_agent.get_completion(fallback_run_config, max_tries=4)
...
@patch("seer.automation.autofix.autofix_agent.AutofixAgent._get_completion")
def test_get_completion_retries_truncates_with_fallback_config(
    mock_get_completion,
    autofix_agent,
    fallback_run_config,
):
    """Test get_completion retries and truncates with fallback models configuration."""
    input_too_long_exception = Exception(
        "Error message: Prompt is too long and exceeds the context window limit."
    )
    mock_success_response = LlmGenerateTextResponse(
    ...
    result = autofix_agent.get_completion(fallback_run_config)
    ...
    assert first_call_args.args[0] == fallback_run_config
    assert first_call_args.kwargs["messages"] == initial_memory, "First call used original memory"

    assert second_call_args.args[0] == fallback_run_config
    assert (
        second_call_args.kwargs["messages"] == expected_truncated_memory
    ), "Second call did not use the correctly truncated memory"
