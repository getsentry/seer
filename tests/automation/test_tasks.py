...
from seer.automation.codegen.models import CodegenStatus, CodegenBaseRequest
...
def test_check_github_reactions_spawns_tasks():
    ...
    with patch('seer.db.Session') as mock_session, \
         patch('seer.automation.tasks.check_pr_reactions.delay') as mock_delay:
        mock_session.return_value.__enter__.return_value.query.return_value.filter.return_value.all.return_value = mock_states
...
def test_check_pr_reactions():
    ...
    with patch('seer.db.Session') as mock_session, \
        ...
        mock_session.return_value.__enter__.return_value.query.return_value.get.return_value = mock_db_state
