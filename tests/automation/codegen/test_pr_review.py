...
from seer.automation.codegen.pr_review_coding_component import format_output
...
class TestFormatResponse(unittest.TestCase):
    def test_format_response_valid_input(self):
        with open("fixtures/output.txt", "r") as file:
            fixture = file.read()
        output = format_output(fixture)
    ...
    def test_format_response_invalid_input(self):
        invalid_fixture = "<comments>[{\"path\": \"src/file.py\", \"line\": 10}]</comments>"

        with self.assertRaises(ValueError) as context:
            format_output(invalid_fixture)
