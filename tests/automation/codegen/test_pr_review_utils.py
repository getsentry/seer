...
from seer.automation.codegen.pr_review_utils import PrReviewUtils, SIMILARITY_THRESHOLD
...
class TestPrReviewUtils(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_model.encode.return_value = np.array([0.1, 0.2, 0.3])  # Simplified embedding vector
        ...
        self.session_patcher = patch.object(Session, '__call__', return_value=self.session_context_mock)
    ...
    def test_is_positive_comment_with_not_enough_similar_comments(self):
        ...
        result = PrReviewUtils.is_positive_comment("Unique comment with few similar ones", "test-owner")
    ...
    def test_is_positive_comment_with_equal_split(self):
        ...
        mock_comments = [MagicMock(is_good_pattern=True), MagicMock(is_good_pattern=False)] * 2 + [MagicMock(is_good_pattern=True)]
        self.session_mock.scalars.return_value.all.return_value = mock_comments
        
        # Call the method (3 positive out of 5 should be True)
        result = PrReviewUtils.is_positive_comment("Comment with equal positive/negative patterns", "test-owner")
        self.assertTrue(result)