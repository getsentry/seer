...
from docker.seer import aggregate_anomalies, process_output
...
class TestSeer(unittest.TestCase):
    def test_aggregate_anomalies(self):
        ...
        granularity = 300
        expected_output = [
            {'start': 1644367729, 'end': 1644367723, 'confidence': 'high', 'received': 10.9, 'expected': 10.5, 'id': 0},
        ]
    ...
    def test_process_output(self, mock_aggregate_anomalies):
        ...
        expected_output = {
            'y': {
                'data': [(1, [{'count': 5.4}]), (1, [{'count': 5.5}])], 'start': 1, 'end': 1
            },
            'yhat_upper': {
                'data': [(1, [{'count': 5.3}]), (1, [{'count': 5.3}])], 'start': 1, 'end': 1
            },
            'yhat_lower': {
                'data': [(1, [{'count': 5.3}]), (1, [{'count': 5.3}])], 'start': 1, 'end': 1
            },
